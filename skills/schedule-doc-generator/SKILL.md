...
- **生成模式（Generate）**：該排程尚未有對應的 `docs/schedule/SCH-XXX-*.md` 文件 → 從零產生完整文件，並分配下一個未使用的 SCH 編號。
- **更新模式（Update）**：該排程已有對應文件 → 切換為「智慧增量更新」，僅修改差異段落，保留其餘內容與使用者自訂修改。
...
> 偵測到 `SCH-002-月結報表產生.md` 對應此排程，將進入**更新模式**並執行智慧增量更新。

或：

> 未偵測到對應文件，將分配編號 SCH-004 並進入**生成模式**。
...
2. **以差異為單位**：比對「最新原始碼 / 設定 / 口頭描述」與「既有文件描述」，列出差異點清單後才動工
3. **只改必要段落**：使用 `str_replace` 精確替換，**不重寫整份文件**，未受影響的章節保持原樣
4. **保留使用者自訂**：辨識並保留 `<!-- 自訂內容開始 -->` ~ `<!-- 自訂內容結束 -->` 區塊；對於明顯人工撰寫的段落（如團隊備註、實際維運經驗），優先保留，必要時提示使用者再次確認
5. **同步更新 README**：若排程屬性（名稱、頻率、可重複執行性、風險等級）有變動，須同步更新 `README.md` 的對應列
6. **必填變更紀錄**：在文件「## 8. 變更紀錄」表格新增一筆，包含日期、新版本號、變更內容、變更者
7. **更新表頭日期**：將表頭表格的「最後更新日期」改為當日
...
1. **偵測模式**：對每個待文件化的排程，依「智慧更新模式」章節判斷進入生成或更新模式（必執行的第一步）
2. **蒐集資訊**：從使用者提供的程式碼、設定檔或描述中擷取排程相關資訊
3. **分析程式碼**：如有原始碼，分析排程邏輯、副作用、冪等性
4. **產生 / 更新文件**：
   - 生成模式：依照模板產生完整的排程說明文件
   - 更新模式：依「比對與差異識別流程」執行精確的 `str_replace`，不重寫全文，並保留 `<!-- 自訂內容 -->` 區塊
5. **冪等性分析**：依照分析指引，完成或重新評估第 4 節的冪等性評估
6. **儲存檔案**：將文件儲存至 `docs/schedule/` 目錄
7. **更新 README**：依模式同步 `README.md`
   - 生成模式：新增對應的索引列與「重複執行安全性總覽」列
   - 更新模式：若屬性（名稱、頻率、可重複執行性、風險等級）有異動，更新對應列
8. **更新變更紀錄**（更新模式必填）：在文件「## 8. 變更紀錄」新增一筆，並把表頭「最後更新日期」改為當日
9. **提示使用者**：標記所有 `[待補充]` 的項目，提醒使用者補充
