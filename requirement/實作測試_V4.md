...
- 使用Maven做為專案管理工具，並以revision提供相關版本資訊
- 使用JDK 25
...
- 加入actuator功能，開啟所有的endpoint，並在info中包含git相關資訊。
...
- 模組包含部門(Department)，員工(Employee)，HR(HumanResources)
- 員工有職級(Rank)與狀態(Status)
- 職級有一般，組長，經理，狀態有在職、未在職。未在職的員工不屬於任何部門，同一部門只能有一個經理。
- HR對於員工可以雇用，分派，解雇
- 部門對於員工可以調職，升職，降級；這些操作會影響職級與狀態，但變更後需要通知HR以便記錄
- event的傳遞使用jdbc
